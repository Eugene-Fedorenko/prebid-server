package sonobi

import (
	"encoding/json"
	"github.com/eugene-fedorenko/prebid-server/openrtb_ext"
	"testing"
)

// This file actually intends to test static/bidder-params/sonobi.json
//
// These also validate the format of the external API: request.imp[i].ext.sonobi

// TestValidParams makes sure that the sonobi schema accepts all imp.ext fields which we intend to support.
func TestValidParams(t *testing.T) {
	validator, err := openrtb_ext.NewBidderParamsValidator("../../static/bidder-params")
	if err != nil {
		t.Fatalf("Failed to fetch the json-schemas. %v", err)
	}

	for _, validParam := range validParams {
		if err := validator.Validate(openrtb_ext.BidderSonobi, json.RawMessage(validParam)); err != nil {
			t.Errorf("Schema rejected sonobi params: %s", validParam)
		}
	}
}

// TestInvalidParams makes sure that the sonobi schema rejects all the imp.ext fields we don't support.
func TestInvalidParams(t *testing.T) {
	validator, err := openrtb_ext.NewBidderParamsValidator("../../static/bidder-params")
	if err != nil {
		t.Fatalf("Failed to fetch the json-schemas. %v", err)
	}

	for _, invalidParam := range invalidParams {
		if err := validator.Validate(openrtb_ext.BidderSonobi, json.RawMessage(invalidParam)); err == nil {
			t.Errorf("Schema allowed unexpected params: %s", invalidParam)
		}
	}
}

var validParams = []string{
	`{"TagID": "123"}`,
}

var invalidParams = []string{
	`{"unit": "123"}`,
}
